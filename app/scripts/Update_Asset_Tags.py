        ...
        try:
            try:
                ...
                if put_response.status_code in [200, 201, 204]:
                    ...
                    log(f"[Row {row_num}] Updated Asset Name {asset_name} and asset id {asset_id} with tags {updated_tags}")
                else:
                    status = f"Failed: {put_response.status_code}"
                    response_str = put_response.text[:500]
                    log(f"[Row {row_num}] Failed to update Asset Name {asset_name} and asset id {asset_id} with tags {updated_tags}: {put_response.status_code}")
                ...
                if hasattr(e, 'response') and e.response is not None:
                    ...
                    response_str = str(e)
                log(f"[Row {row_num}] Error for Asset Name {asset_name} and asset id {asset_id}: {response_str[:100]}")
