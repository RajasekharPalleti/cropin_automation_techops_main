"""
Updates the Name for Croppable Areas (CA).

Inputs:
Excel file with the following columns:
- CA_id
- name  (new CA name to set, e.g. "AssetSplits updated|CA-1|1")
"""
import json
import requests
import pandas as pd
    ...
    for index, row in df.iterrows():
        try:
            try:
                ...
                if pd.isna(CA_id) or pd.isna(new_name):
                ...
                # Update Name ONLY
                # -----------------------
                CA_data["name"] = str(new_name)
                log(f"✏️ Setting name: {new_name}")
                ...
                if put_response.status_code != 200:
                    ...
                    df.at[index, "CA_Response"] = put_response.text
                    log(f"✅ Successfully updated name for CA_ID: {CA_id}")
